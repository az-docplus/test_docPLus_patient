...
function LandingPage({ navigation }) {
      ...
      const temp = doctors.filter((item, index) => {
        ...
        if (
          item?.specialty?.toLowerCase() ==
            ActiveSpeciality.item.toLowerCase() &&
  ...
  const applyAllFilter = (e) => {
    ...
    if (typeFilter !== '') {
      payload.consultationType = typeFilter;
    }
    if (location !== '') {
    ...
    if (ratings !== 'Any') {
      payload.ratings = ratings.split('+')[0];
    }
    if (exp !== 'Any') {
      payload.experience = exp.split('+')[0];
    }
    if (GenderFilter !== '') {
    ...
    if (specialityList.length > 0) {
      payload.specialties = specialityList;
    }
    if (languages.length > 0) {
      payload.languages = languages;
    }
    if (NowFilter !== '') {
      payload.is_superDoc = NowFilter === 'Now';
    }
      ...
      .then((res) => {
        ...
        if (fav) {
          ...
          let result = data.filter((o1) =>
            patient?.favourites.some((o2) => o1._id === o2._id),
          );
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            if (!specialityList.includes(item)) {
                              ...
                              const selectedAcc = specialityList.filter((a) => {
                                if (a === item) {
                                  return false;
                                }
                                return true;
                      ...
                      {['Favourites'].map((item, index) => {
                        return (
                          <TouchableOpacity
                            key={index}
                            style={{
                              backgroundColor: fav ? '#e0f2f2' : '#eaeaea',
                              paddingHorizontal: '6%',
                              paddingVertical: '1%',
                              paddingBottom: '3%',
                              borderRadius: 5,
...
function EmptyListComponent({ theme, loading }) {
  ...
  return (
    <View style={elStyles.container}>
      ...
      />
      <Text
        style={[elStyles.text, { color: Colors.primary_text_color[theme] }]}>
