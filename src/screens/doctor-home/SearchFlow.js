...
export default function SearchFlow({
  ...
  const handleSearch = (searchKey) => {
    const diseases = doctors.map((item) => item.diseases);
    const searchParams = {
  ...
  const [rescentSearch, setRescentSearch] = useState([]);

  const getRescentSearch = async () => {
    const data = await AsyncStorage.getItem('rescenrSearch');
    let res = JSON.parse(data);
    if (res != null) {
      setRescentSearch(res);
    }
  };
                      ...
                      <TouchableOpacity
                        ...
                        <View
                          style={{
                            ...
                            marginRight: 10,
                            borderWidth:
                              doctors.map((item) => item.specialty) ===
                              item.name
                                ? 1
                                : 0,
                            borderColor: '#EA1A65',
