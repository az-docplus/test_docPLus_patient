...
const Settings = ({params}) => {
  ...
  const [followUps, setFollowUps] = useState(7);
  const {
    patient,
    records,
    gettingRecords,
    ...
    <>
      ...
      {false ? (
        ...
        <FlatList
          ...
          ListFooterComponent={
            <NewItem icon={false} text="Save" onPress={() => {
                const followUpsTemp = patient?.followUps ?? {}
                // const followUpsTemp = {}
                followUpsTemp[`${userData?._id}`] = followUps
                console.log(followUpsTemp, patient?.followUps,"DKLFJDFJDK")
                dispatch(UpdatePatient({
                    followUps: followUpsTemp
                }, patient?._id, (res) => {
                    console.log(res?.followUps, "djfdlfj")
                }, (err) => {
                    console.log(err, "slfjdslfjdf")
                }))
            }} />
          }
          renderItem={({item}) => {
              ...
              <View
                ...
                //   flexDirection: 'row',
                  paddingVertical: 15,
                    ...
                    <TextInput>
                        Select the number of Follow up days
                    </TextInput>
                    <Picker
                selectedValue={followUps}
                style={{width: '100%'}}
                onValueChange={(itemValue, itemIndex) =>
                  setFollowUps(itemValue)
                }>
                <Picker.Item color={'#8e9393'} label="Select Follow up days" value="" />
                {[1,2,3,4,5,6,7].map((item, index) => {
                  return <Picker.Item label={item === 1 ? `${item} Day`: `${item} Days`} value={item} />;
                })}
              </Picker>
                {/* <ReportsItem
                editMode={editMode}
