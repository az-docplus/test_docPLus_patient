...
import ReferItem from '../../../components/molecules/MedicalHistory/ReferItem';
...
const Refer = ({ params }) => {
  ...
  useEffect(() => {
    ...
    Axios.get(`${Host}/doctors/notifications/get/${patient?._id}`)
      .then((response) => {
        console.log(response.data.data, "sdsfjdslfjdf");
        setState(response.data.data);
        setAllState(response.data.data);
        //setMarkedAsRead(response.data.data.read)
  ...
  const onEndEditing = (search) => {
    if (search === '') setState(allState)
    else {
      const c = allState.filter((p, id) => {
        if (
          (p?.for?.firstName).toLowerCase().includes(search.toLowerCase())
           || (p?.for?.lastName).toLowerCase().includes(search.toLowerCase())
    ...
    <>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View
            ...
            <Text style={{ textAlign: "center", fontFamily: "Montserrat-Medium", color: Colors.primary_text_color[theme], fontSize: 20 }}>
              ...
              {Local('doctor.medical_history.No Refferal Found')}
											</Text>
          </View>
        }
        ...
        renderItem={({ item }) => {
            ...
            if(item?.by?._id == userData?._id && !item?.read && new Date().getHours() - new Date(item?.when).getHours() < 24) {
                console.log(item?.appointment, "dslfjsdlfjd")
                return <ReferItem
                editMode={editMode}
                refresh={refresh}
